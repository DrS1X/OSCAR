...
bool gdalOpt::readGeoTiff(const char* in_fileName, double* pTiffData) {
...
bool gdalOpt::readGeoTiff(const string file, int* pBuffer) {
...
bool gdalOpt::writeGeoTiff(string fileName, Meta meta, int *buf) {
...
bool gdalOpt::writeGeoTiff(string fileName, Meta meta, double *buf) {
	GDALAllRegister();
	CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "NO");
	if (geoTiffDriver)
	{
...
bool gdalOpt::ReadFileByGDAL(const char * mFileName)
{
	/*GDALAllRegister();
	CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "NO");
	...
	if (pDataSet != NULL)
	{
		...
		if (metadatas != NULL)
		{
			...
			dsName = opt::CStrToQStr(pHdf->GetDatasetsNameByIndex(opt::QStrToCStr(file),0)).toStdString();
...
bool gdalOpt::Convert_GeoTiff2HDF(const char* in_fileName, const char* out_fileName, double startLat, double endLat, double startLog, double endLog)
{
...
const char * gdalOpt::GetProjection(const char * mFileName)
{
	GDALAllRegister();
	CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "NO");
	pDataSet = (GDALDataset*)GDALOpen(mFileName, GA_ReadOnly);
