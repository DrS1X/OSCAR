...
bool tiffOpt::readGeoTiff(const char* in_fileName, double* pTiffData) {
...
bool tiffOpt::readGeoTiff(const string file, int* pBuffer) {
...
bool tiffOpt::writeGeoTiff(string fileName, Meta meta, int *buf) {
...
bool tiffOpt::writeGeoTiff(string fileName, Meta meta, double *buf) {
	GDALAllRegister();
	CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "NO");
	if (geoTiffDriver)
	{
		...
		min_max = opt::GetMin_Max(min_max, buf, iRow, iCol, fillValue);
		minValue = min_max[0]; maxValue = min_max[1];
		stdValue = opt::GetStdValue(buf, iRow, iCol, fillValue);
		meanValue = opt::GetMeanValue(buf, iRow, iCol, fillValue);
...
bool tiffOpt::Convert_GeoTiff2HDF(const char* in_fileName, const char* out_fileName, double startLat, double endLat, double startLog, double endLog)
{
	...
	if (pDataSet != NULL)
	{
		...
		if (metadatas != NULL)
		{
			...
			{
				tempStr.clear();
				opt::split(mdArr->at(i), s2, &tempStr);
		...
		//}
		double *min_max = new double[2];
		min_max = opt::GetMin_Max(min_max, pGeoData, iRow, iCol, fillValue);
		minValue = min_max[0];
		maxValue = min_max[1];
		stdValue = opt::GetStdValue(pGeoData, iRow, iCol, fillValue);
		meanValue = opt::GetMeanValue(pGeoData, iRow, iCol, fillValue);
		CPLFree(pTiffData);
	}	
...
const char * tiffOpt::GetProjection(const char * mFileName)
{
	GDALAllRegister();
	CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "NO");
...
void tiffOpt::save(string outputPath, string startTime, AnomalyType anomalyType, vector<Poly>& polygons){
...
void tiffOpt::save(string outPath, string startTime, string abnormalType, const double startLog, const double startLat, const double resolution, vector<Poly>& polygons) {
    GDALAllRegister();
    //保存shp
    CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "NO");
